...
export default class AuthController {
  ...
  login = async (request: any, response: any) => {
    try {
      ...
      const data: RefreshTokenType | Error = await this.authService.login(
        login,
        password
      );
  ...
  register = async (request: any, response: any) => {
    const { body } = request;
    try {
      const data: UserAttributes | Error = await this.authService.register(
        body
      );
      response.status(201).send(data);
    } catch (error: any) {
  ...
  refreshToken = async (request: any, response: any) => {
    ...
    try {
      const data: RefreshTokenType | Error =
